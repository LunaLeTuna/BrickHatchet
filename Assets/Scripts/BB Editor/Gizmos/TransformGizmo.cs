...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		void Awake()
		{
            ...
            SettingsManager.SettingsChanged.AddListener(LoadSettings);
            LoadSettings();
		}
		...
		IEnumerator TransformSelected(TransformType transType)
		{
			...
			while(!Mouse.current.leftButton.wasReleasedThisFrame)
			{
				...
				if(previousMousePosition != Vector3.zero && mousePosition != Vector3.zero)
				{
					...
					{
                        // TODO: Scale from edges (not center), work w/ negative and positive vectors properly
		...
		void GetTarget()
		{
			if(nearAxis == Axis.None && Mouse.current.leftButton.wasPressedThisFrame)
			{
				...
				RaycastHit hitInfo; 
				if(Physics.Raycast(myCamera.ScreenPointToRay(Mouse.current.position.ReadValue()), out hitInfo, Mathf.Infinity, selectionMask))
				{
			...
			if(target != null)
			{
				...
				for(int i = 0; i < renderersBuffer.Count; i++)
				{
					...
					if(!highlightedRenderers.Contains(render))
					{
						...
						materialsBuffer.AddRange(render.materials);

						for (int j = 0; j < materialsBuffer.Count; j++) {
                            (Color, MaterialLibrary.FaceType, Vector2, bool, bool) materialInfo = MaterialLibrary.GetMaterialInfo(materialsBuffer[j]);
                            if (materialInfo.Item5) {
                                materialsBuffer[j] = MaterialLibrary.GetMaterial(materialInfo.Item1, materialInfo.Item2, materialInfo.Item3, true);
                            } else {
                                materialsBuffer[j].shader = Shader.Find("Brick/Wireframe");
                            }
                        }
			...
			for(int i = 0; i < renderersBuffer.Count; i++)
			{
				...
				if(render != null)
				{
					...
					for (int j = 0; j < materialsBuffer.Count; j++) {
                        (Color, MaterialLibrary.FaceType, Vector2, bool, bool) materialInfo = MaterialLibrary.GetMaterialInfo(materialsBuffer[j]);
                        if (materialInfo.Item5) {
                            materialsBuffer[j] = MaterialLibrary.GetMaterial(materialInfo.Item1, materialInfo.Item2, materialInfo.Item3, false);
                        } else {
                            materialsBuffer[j].shader = Shader.Find("Brick/Normal");
                        }
                    }
